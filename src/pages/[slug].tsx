...
export default function Page({ heading, body, all }: PageTemplateProps) {
  const router = useRouter();

  // retorna um loading, qq coisa enquanto tá sendo criado
  if (router.isFallback) return null;

  return <PageTemplate heading={heading} body={body} all={all} />;
}
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  const all = await api.get(`all`).then(({ data }) => {
